...
TCPSender::TCPSender(const size_t capacity, const uint16_t retx_timeout, const std::optional<WrappingInt32> fixed_isn)
    ...
    , _window_size(1)
    , _ackno(fixed_isn->raw_value())
    , _stream(capacity)
    , _timer(false, retx_timeout) {}

uint64_t TCPSender::bytes_in_flight() const { return next_seqno() - _ackno; }
...
void TCPSender::ack_received(const WrappingInt32 ackno, const uint16_t window_size) {
    //! 要忽略哪些无效的ackno
    if (next_seqno() < ackno || ackno < _ackno) return;

    _ackno = ackno, _window_size = window_size;  // 追踪最新的ackno和window_size

    // 查看outstanding segments的数据结构，移除已经被确认的，其实就是ackno一下数字的
    auto orgin_size = _cache.size();
    auto it = _cache.lower_bound(unwrap(ackno, _isn, _stream.bytes_written()));
