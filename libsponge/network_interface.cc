...
void NetworkInterface::send_datagram(const InternetDatagram &dgram, const Address &next_hop) {
    ...
    //     header.dst = ETHERNET_BROADCAST;  // 配置广播地址
    //     frame.header() = header;
...
optional<InternetDatagram> NetworkInterface::recv_frame(const EthernetFrame &frame) {
    ...
    //         {
    //             send_rely(message.sender_ip_address, message.sender_ethernet_address);
    //         } else if (message.opcode == ARPMessage::OPCODE_REPLY &&
    //                    message.target_ethernet_address == _ethernet_address) { // 是ARP回复，且是自己需要的
...
EthernetFrame NetworkInterface::warp_Ethernet(uint16_t type,EthernetAddress src,EthernetAddress dst,uint32_t s_ip,uint32_t d_ip,std::optional<InternetDatagram>dgram,uint16_t arp_type){
    ...
    if (type==1)
    {
        ...
        message.target_ethernet_address=dst;
        frame.header() = header;
        frame.payload()=message.serialize();
        return frame;
    }
